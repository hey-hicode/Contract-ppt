          ...
          <nav className="space-y-2 text-body-color text-base leading-relaxed!">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`flex items-center gap-4 rounded-md px-3 py-3 hover:bg-black/5 dark:hover:bg-white/10 ${pathname === link.href ? "text-primary font-medium bg-primary/5" : ""
                  }`}
              >
          ...
          <header className="sticky top-0 z-20 flex h-[80px] items-center justify-between border-b border-gray-200/50 dark:border-white/10 bg-white/80 dark:bg-gray-dark/80 backdrop-blur-md px-6 py-4 shadow-sm transition-all duration-300">
            ...
            <div className="flex items-center gap-4">
              ...
              <UserButton
                appearance={{
                  elements: {
                    avatarBox: "h-9 w-9 ring-2 ring-white dark:ring-white/10 shadow-md"
                  }
                }}
              />
            </div>
