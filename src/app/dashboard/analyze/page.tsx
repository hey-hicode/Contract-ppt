...
> = {
  idle: {
    ...
    className: "border-gray-300 dark:border-white/15 bg-white dark:bg-white/5 text-gray-700 dark:text-slate-200",
    icon: Clock,
  },
  uploading: {
    label: "Uploading",
    className: "border-amber-400/40 bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-100",
    icon: Loader2,
    spin: true,
  },
  processing: {
    label: "Processing",
    className: "border-sky-400/40 bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-100",
    icon: Loader2,
    spin: true,
  },
  completed: {
    label: "Ready to review",
    className: "border-emerald-400/40 bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-100",
    icon: CheckCircle2,
  },
  error: {
    label: "Upload failed",
    className: "border-rose-400/40 bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-100",
    icon: XCircle,
  },
};
...
{statusMessage}</p>
                ...
                <div className="rounded-3xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 ">
                  ...
                  <ScrollArea className="mt-4 h-[26rem] w-full rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-4">
                    {sections.length > 0 ? (
                      <div className="space-y-4">
                        {sections.map((section, index) => (
                          <div
                            key={`${index}-${section.slice(0, 12)}`}
                            className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-4 text-sm leading-6 text-gray-700 dark:text-slate-200/85 shadow-sm"
                          >
                            <span className="mb-2 inline-flex items-center gap-2 rounded-full border border-gray-300 dark:border-white/10 bg-gray-100 dark:bg-white/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-gray-700 dark:text-sky-200">
                              <FileText className="size-3 text-sky-600 dark:text-sky-200" />
                              Section {index + 1}
                            </span>
                      ...
                      <div className="flex h-full items-center justify-center text-sm text-gray-500 dark:text-slate-400/70">
