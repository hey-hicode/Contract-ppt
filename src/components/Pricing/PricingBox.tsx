const PricingBox = (props: {
  ...
  const { price, duration, packageName, subtitle, custom, children } = props;

  return (
    <div className="w-full ">
      <div className="shadow-three h-[600px] hover:shadow-one dark:bg-gray-dark dark:shadow-two dark:hover:shadow-gray-dark relative z-10 rounded-xs bg-white px-8 py-10">
        <div className="flex items-center justify-between">
          {custom ? (
            ...
            <h3 className="price mb-2 text-[32px] font-bold text-black dark:text-white">
              ...
              <span className="time text-black/50 text-sm font-bold">
                /{duration}
              </span>
            </h3>
          )}
        ...
        <div className="border-body-color/10 mb-8 border-b pb-8 dark:border-white/10">
          <button className="bg-primary/80 hover:shadow-signUp flex w-full items-center justify-center rounded-xs p-3 text-base font-semibold text-white transition duration-300 ease-in-out">
            {custom ? "Contact Us" : "Start Free Trial"}
          </button>
        </div>
        <div>{children}</div>
